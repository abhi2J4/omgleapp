...
io.on("connection", (socket) => {
    ...
    socket.on("disconnect", () => {
        ...
        var disUser = userConnection.find((p) => p.connectedId = socket.id);
        if (disUser) {
            userConnection = userConnection.filter((p) => p.connectedId = !socket.id);
            console.log("Rest users username are :", userConnection.map(function (user) {
                return user.user_id;     
            }));  
        }             
    });  
    ...
    //     if (closedUser) {
    //         console.log("closedUser user is:", candidateReceiver.connectionId);
